...
8. **Не успела сделать на текущий момент**. Все, что указано в этом пункте, актуально на тот момент, когда Вы это читаете.
    * :fire: алгоритм рекомендованных переговорок (полностью готов алгоритм, корректно отображаются события в режиме event и new, осталось отобразить рекомендации в режиме make)
