...
8. **Не успела сделать на текущий момент**. Все, что указано в этом пункте, актуально на тот момент, когда Вы это читаете. Я буду продолжать делать коммиты после сдачи анкеты, чтобы привести это приложение к законченному рабочему состоянию. 
    * редактирование существующих встреч ( измененные данные не сохраняются в БД )
    * алгоритм рекомендованных переговорок (выводятся все комнаты, все подготовлено для реализации алгоритма)
    * запрет создания встреч в прошлом (через редактор или плюсик)
    * запрет редактирования встреч в прошлом
    * дополнить код комментариями, сделать рефакторинг
    * нет модального окна с сообщением об удалении встречи, нет автоматического перехода на главную после удаления
    * написать тесты
